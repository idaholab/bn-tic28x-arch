...
bool VbitflipVra::Text(const uint8_t* data, uint64_t addr, size_t& len,
                       std::vector<BN::InstructionTextToken>& result,
                       const AddressMode amode) {
  const auto dataOp = DataToOpcode(data, GetLength());
  const auto regA = GetRegA(dataOp);
...
bool Vlshl32Vra5bit::Text(const uint8_t* data, uint64_t addr, size_t& len,
                          std::vector<BN::InstructionTextToken>& result,
                          const AddressMode amode) {
  const auto dataOp = DataToOpcode(data, GetLength());
  const auto regA = GetRegA(dataOp);
...
bool Vlshr32Vra5bit::Text(const uint8_t* data, uint64_t addr, size_t& len,
                          std::vector<BN::InstructionTextToken>& result,
                          const AddressMode amode) {
  const auto dataOp = DataToOpcode(data, GetLength());
  const auto regA = GetRegA(dataOp);
...
// VCU - Negate instruction

bool VnegVra::Text(const uint8_t* data, uint64_t addr, size_t& len,
                   std::vector<BN::InstructionTextToken>& result,
                   const AddressMode amode) {
  const auto dataOp = DataToOpcode(data, GetLength());
  const auto regA = GetRegA(dataOp);
...
bool VccmacVr5Vr4Vr3Vr2Vr1Vr0::Text(
    const uint8_t* data, uint64_t addr, size_t& len,
    std::vector<BN::InstructionTextToken>& result, const AddressMode amode) {
  len = GetLength();
...
bool VccmacVr5Vr4Vr3Vr2Vr1Vr0Vmov32VraMem32::Text(
    const uint8_t* data, uint64_t addr, size_t& len,
    std::vector<BN::InstructionTextToken>& result, const AddressMode amode) {
  len = GetLength();
...
bool VccmpyVr3Vr2Vr1Vr0Vmov32VraMem32Load::Text(
    const uint8_t* data, uint64_t addr, size_t& len,
    std::vector<BN::InstructionTextToken>& result, const AddressMode amode) {
  len = GetLength();
...
bool VcconVra::Text(const uint8_t* data, uint64_t addr, size_t& len,
                    std::vector<BN::InstructionTextToken>& result,
                    const AddressMode amode) {
  const auto dataOp = DataToOpcode(data, GetLength());
  const auto regA = GetRegA(dataOp);
...
bool Vcdsub16Vr6Vr4Vr3Vr2::Text(const uint8_t* data, uint64_t addr, size_t& len,
                                std::vector<BN::InstructionTextToken>& result,
                                const AddressMode amode) {
  len = GetLength();
...
bool Vcdsub16Vr6Vr4Vr3Vr2Vmov32VraMem32::Text(
    const uint8_t* data, uint64_t addr, size_t& len,
    std::vector<BN::InstructionTextToken>& result, const AddressMode amode) {
  len = GetLength();
...
bool VcmacVr7Vr6Vr5Vr4Mem32Xar7Postinc::Text(
    const uint8_t* data, uint64_t addr, size_t& len,
    std::vector<BN::InstructionTextToken>& result, const AddressMode amode) {
  const auto dataOp = DataToOpcode(data, GetLength());
  const auto mem32 = GetMem32(dataOp);
...
bool Vcshl16Vra4bit::Text(const uint8_t* data, uint64_t addr, size_t& len,
                          std::vector<BN::InstructionTextToken>& result,
                          const AddressMode amode) {
  const auto dataOp = DataToOpcode(data, GetLength());
  const auto regA = GetRegA(dataOp);
  const auto imm4 = GetImm4(dataOp);
  len = GetLength();
  ...
  RegText(RegTextInfo{.regnum = static_cast<uint8_t>(Registers::VR0 + regA)},
          result);
  LshiftText(result);
  ConstText(ConstTextInfo{.value = imm4, .nbits = 4}, result);
