...
// ============================================================================
// Vcdsub16Vr6Vr4Vr3Vr2Vmov32VraMem32 Lift Tests
// ============================================================================
//
// VCDSUB16 VR6, VR4, VR3, VR2 || VMOV32 VRa, mem32
// 4-byte instruction.  Encoding:
//   LSW (bits [31:16]): 0xE3FB (fixed)
//   MSW (bits [15:0]):  0000 aaaa mmmm mmmm
//     bits [11:8] = VRa index (0-7, not 6 or 8)
//     bits [7:0]  = mem32 addressing code
//
// Lifted as two straight-line IL instructions (no branching):
//   Instruction 0: VCDSUB16 intrinsic
//   Instruction 1: Parallel VMOV32 -- LLIL_SET_REG(VRa, LLIL_LOAD(mem32))

class Vcdsub16Vr6Vr4Vr3Vr2Vmov32VraMem32LiftTest
    : public ::testing::TestWithParam<RegAMem32TestCase> {};

TEST_P(Vcdsub16Vr6Vr4Vr3Vr2Vmov32VraMem32LiftTest, HelperFunctionsWork) {
  const auto& tc = GetParam();
  const uint32_t opcode =
  ...
  // 4-byte instruction
  EXPECT_EQ(TIC28X::Vcdsub16Vr6Vr4Vr3Vr2Vmov32VraMem32().GetLength(), 4u);

  // GetRegA round-trips (4-bit field at bits [11:8])
  EXPECT_EQ(TIC28X::Vcdsub16Vr6Vr4Vr3Vr2Vmov32VraMem32::GetRegA(opcode),
            tc.regA & 0xF);

  // GetMem32 round-trips (8-bit field at bits [7:0])
  EXPECT_EQ(TIC28X::Vcdsub16Vr6Vr4Vr3Vr2Vmov32VraMem32::GetMem32(opcode),
            tc.mem32 & 0xFF);
}

static const auto vcdsub16_vr6_vr4_vr3_vr2_vmov32_vra_mem32_test_cases =
    ::testing::Values(RegAMem32TestCase{0, 0x00, "VR0_mem_0"},
                      RegAMem32TestCase{1, 0xAB, "VR1_mem_ab"},
                      RegAMem32TestCase{3, 0xFF, "VR3_mem_ff"},
                      RegAMem32TestCase{5, 0x42, "VR5_mem_42"},
                      RegAMem32TestCase{7, 0x01, "VR7_mem_01"});

INSTANTIATE_TEST_SUITE_P(Vcdsub16Vr6Vr4Vr3Vr2Vmov32VraMem32,
                         Vcdsub16Vr6Vr4Vr3Vr2Vmov32VraMem32LiftTest,
                         vcdsub16_vr6_vr4_vr3_vr2_vmov32_vra_mem32_test_cases,
                         NameFromParam{});

// IL verification: two IL instructions (non-branching).
// Instruction 0: vcdsub16 -- TIC28X_INTRIN_VCDSUB16 (LLIL_INTRINSIC)
// Instruction 1: parallel VMOV32 -- LLIL_SET_REG(VRa, LLIL_LOAD(mem32))

class Vcdsub16Vr6Vr4Vr3Vr2Vmov32VraMem32ILTest
    : public ILTestFixture,
      public ::testing::WithParamInterface<RegAMem32TestCase> {};

TEST_P(Vcdsub16Vr6Vr4Vr3Vr2Vmov32VraMem32ILTest, GeneratesCorrectIL) {
  const auto& tc = GetParam();
  const uint32_t opcode =
      TIC28X::Vcdsub16Vr6Vr4Vr3Vr2Vmov32VraMem32::SetRegA(tc.regA) |
      TIC28X::Vcdsub16Vr6Vr4Vr3Vr2Vmov32VraMem32::SetMem32(tc.mem32);
  ...
  TIC28X::Vcdsub16Vr6Vr4Vr3Vr2Vmov32VraMem32 instr;
  size_t len = 4;
  ASSERT_TRUE(instr.Lift(data, 0x1000, len, *il, GetArch()));
  EXPECT_EQ(len, 4u);
  ...
  // === Instruction 0: VCDSUB16 intrinsic ===
  const auto vcdsub16Expr = il->GetRawExpr(il->GetIndexForInstruction(0));
  EXPECT_EQ(vcdsub16Expr.operation, LLIL_INTRINSIC);
  EXPECT_EQ(vcdsub16Expr.operands[2],
            TIC28X::TIC28X_INTRIN_VCDSUB16_VR6_VR4_VR3_VR2)
      << "First instruction should be the vcdsub16 intrinsic";
...
// ============================================================================
// VcmacVr5Vr4Vr3Vr2Vr1Vr0 Lift Tests
// ============================================================================
//
// VCMAC VR5, VR4, VR3, VR2, VR1, VR0 (opcode 0xE501, exact 2-byte match,
// no variable fields). Complex Multiply and Accumulate.
// Lifted as a single LLIL_INTRINSIC named "vcmac" with:
//   inputs:  VR0, VR1, VR2, VR3, VR4, VR5, VSTATUS
//   outputs: VR5, VR4, VR3, VR2, VSTATUS

// Test that the intrinsic ID and name are correctly registered.
TEST(VcmacVr5Vr4Vr3Vr2Vr1Vr0Intrinsic, IntrinsicIsDefined) {
  EXPECT_EQ(TIC28X::TIC28X_INTRIN_VCMAC_VR5_VR4_VR3_VR2_VR1_VR0, 9u);
}

TEST(VcmacVr5Vr4Vr3Vr2Vr1Vr0Intrinsic, IntrinsicName) {
  auto arch = std::make_unique<TIC28X::TIC28XArchitecture>("tic28x-vcmac-test");
  EXPECT_EQ(arch->GetIntrinsicName(
                TIC28X::TIC28X_INTRIN_VCMAC_VR5_VR4_VR3_VR2_VR1_VR0),
            "vcmac");
}

TEST(VcmacVr5Vr4Vr3Vr2Vr1Vr0Intrinsic, IntrinsicInputCount) {
  auto arch = std::make_unique<TIC28X::TIC28XArchitecture>("tic28x-vcmac-test");
  auto inputs = arch->GetIntrinsicInputs(
      TIC28X::TIC28X_INTRIN_VCMAC_VR5_VR4_VR3_VR2_VR1_VR0);
  // VR0, VR1, VR2, VR3, VR4, VR5, VSTATUS
  ASSERT_EQ(inputs.size(), 7u) << "vcmac should have 7 inputs";
  EXPECT_EQ(inputs[0].name, "vr0");
  EXPECT_EQ(inputs[1].name, "vr1");
  EXPECT_EQ(inputs[2].name, "vr2");
  EXPECT_EQ(inputs[3].name, "vr3");
...
TEST(VcmacVr5Vr4Vr3Vr2Vr1Vr0Intrinsic, IntrinsicOutputCount) {
  auto arch = std::make_unique<TIC28X::TIC28XArchitecture>("tic28x-vcmac-test");
  auto outputs = arch->GetIntrinsicOutputs(
      TIC28X::TIC28X_INTRIN_VCMAC_VR5_VR4_VR3_VR2_VR1_VR0);
  // VR5 (Re accum), VR4 (Im accum), VR3 (Re mult), VR2 (Im mult), VSTATUS
  ASSERT_EQ(outputs.size(), 5u) << "vcmac should have 5 outputs";
}

TEST(VcmacVr5Vr4Vr3Vr2Vr1Vr0Intrinsic, GetAllIntrinsicsIncludesVcmac) {
  auto arch = std::make_unique<TIC28X::TIC28XArchitecture>("tic28x-vcmac-test");
  auto intrinsics = arch->GetAllIntrinsics();
  EXPECT_NE(std::find(intrinsics.begin(), intrinsics.end(),
                      TIC28X::TIC28X_INTRIN_VCMAC_VR5_VR4_VR3_VR2_VR1_VR0),
            intrinsics.end())
      << "vcmac should be in the intrinsics list";
}

// IL verification test: VCMAC produces a single LLIL_INTRINSIC with the
// correct intrinsic ID.  Lift is straight-line (no branching), so IL tests
// are safe in unit-test contexts.
class VcmacVr5Vr4Vr3Vr2Vr1Vr0ILTest : public ILTestFixture {};

TEST_F(VcmacVr5Vr4Vr3Vr2Vr1Vr0ILTest, GeneratesCorrectIL) {
  // Encode opcode 0xE501 as a 2-byte little-endian byte array.
  uint8_t data[2];
  OpcodeToData2(
      static_cast<uint16_t>(TIC28X::Opcodes::VCMAC_VR5_VR4_VR3_VR2_VR1_VR0),
      data);

  auto il = CreateIL();
  if (!il || !il->GetObject()) GTEST_SKIP() << "BN LLIL unavailable (CI mode)";

  TIC28X::VcmacVr5Vr4Vr3Vr2Vr1Vr0 instr;
  size_t len = 2;
  ASSERT_TRUE(instr.Lift(data, 0x1000, len, *il, GetArch()));
  EXPECT_EQ(len, 2u);
...
// ============================================================================
// VcmpyVr3Vr2Vr1Vr0Vmov32Mem32Vra Lift Tests
// ============================================================================
//
...
class VcmpyVr3Vr2Vr1Vr0Vmov32Mem32VraLiftTest
    : public ::testing::TestWithParam<RegAMem32TestCase> {};

TEST_P(VcmpyVr3Vr2Vr1Vr0Vmov32Mem32VraLiftTest, HelperFunctionsWork) {
  const auto& tc = GetParam();
  const uint32_t opcode =
  ...
  // 4-byte instruction
  EXPECT_EQ(TIC28X::VcmpyVr3Vr2Vr1Vr0Vmov32Mem32Vra().GetLength(), 4u);

  // GetRegA round-trips (4-bit field at bits [11:8])
  EXPECT_EQ(TIC28X::VcmpyVr3Vr2Vr1Vr0Vmov32Mem32Vra::GetRegA(opcode),
            tc.regA & 0xF);

  // GetMem32 round-trips (8-bit field at bits [7:0])
  EXPECT_EQ(TIC28X::VcmpyVr3Vr2Vr1Vr0Vmov32Mem32Vra::GetMem32(opcode),
            tc.mem32 & 0xFF);
}

static const auto vcmpy_vr3_vr2_vr1_vr0_vmov32_mem32_vra_test_cases =
    ::testing::Values(RegAMem32TestCase{0, 0x00, "VR0_mem_0"},
                      RegAMem32TestCase{1, 0xAB, "VR1_mem_ab"},
                      RegAMem32TestCase{4, 0xFF, "VR4_mem_ff"},
                      RegAMem32TestCase{6, 0x42, "VR6_mem_42"},
...
INSTANTIATE_TEST_SUITE_P(VcmpyVr3Vr2Vr1Vr0Vmov32Mem32Vra,
                         VcmpyVr3Vr2Vr1Vr0Vmov32Mem32VraLiftTest,
                         vcmpy_vr3_vr2_vr1_vr0_vmov32_mem32_vra_test_cases,
                         NameFromParam{});
...
class VcmpyVr3Vr2Vr1Vr0Vmov32Mem32VraILTest
    : public ILTestFixture,
      public ::testing::WithParamInterface<RegAMem32TestCase> {};

TEST_P(VcmpyVr3Vr2Vr1Vr0Vmov32Mem32VraILTest, GeneratesCorrectIL) {
  const auto& tc = GetParam();
  const uint32_t opcode =
      TIC28X::VcmpyVr3Vr2Vr1Vr0Vmov32Mem32Vra::SetRegA(tc.regA) |
      TIC28X::VcmpyVr3Vr2Vr1Vr0Vmov32Mem32Vra::SetMem32(tc.mem32);
  ...
  TIC28X::VcmpyVr3Vr2Vr1Vr0Vmov32Mem32Vra instr;
  size_t len = 4;
  ASSERT_TRUE(instr.Lift(data, 0x1000, len, *il, GetArch()));
  EXPECT_EQ(len, 4u);
...
INSTANTIATE_TEST_SUITE_P(VcmpyVr3Vr2Vr1Vr0Vmov32Mem32Vra,
                         VcmpyVr3Vr2Vr1Vr0Vmov32Mem32VraILTest,
                         vcmpy_vr3_vr2_vr1_vr0_vmov32_mem32_vra_test_cases,
                         NameFromParam{});
...
// ============================================================================
//
// VCMPY VR3, VR2, VR1, VR0 || VMOV32 VRa, mem32
// Complex 16x16=32-bit Multiply with parallel 32-bit load.
// Variable fields: VRa (bits [11:8]), mem32 (bits [7:0]).
// Lifted as:
//   Instruction 0: LLIL_INTRINSIC (vcmpy) — same as standalone VCMPY
//   Instruction 1: LLIL_SET_REG(VRa, LLIL_LOAD([mem32]))
...
TEST_P(VcmpyVr3Vr2Vr1Vr0Vmov32VraMem32ILTest, GeneratesCorrectIL) {
  ...
  // === Instruction 1: Parallel VMOV32 VRa = [mem32] — SetReg(VRa, Load(Const))
  // ===
  const auto setRegExpr = il->GetRawExpr(il->GetIndexForInstruction(1));
  EXPECT_EQ(setRegExpr.operation, LLIL_SET_REG)
      << "Second instruction should be LLIL_SET_REG";
  ...
  // Load source address: LLIL_CONST with mem32 value
  const auto addrExpr = il->GetRawExpr(loadExpr.operands[0]);
  EXPECT_EQ(addrExpr.operation, LLIL_CONST);
  EXPECT_EQ(addrExpr.operands[0], tc.mem32 & 0xFF);
}
